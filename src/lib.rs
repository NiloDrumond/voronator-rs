...
impl CentroidDiagram {
    ...
    pub fn from_tuple(coords: &[(f64, f64)]) -> Option<Self> {
        let points: Vec<Point> = coords.iter().map(|p| Point::from_xy(p.0, p.1)).collect();
        CentroidDiagram::new(&points)
    }

    fn calculate_polygons(points: &[Point], centers: &[Point], delaunay: &Triangulation) -> Vec<Polygon> {
...
fn helper_points(polygon: &Polygon) -> Vec<Point> {
    ...
    points.push(Point::from_xy(min.x() - width, min.y() + height / 2.0));
    points.push(Point::from_xy(max.x() + width, min.y() + height / 2.0));
    points.push(Point::from_xy(min.x() + width / 2.0, min.y() - height));
    points.push(Point::from_xy(min.x() + width / 2.0, max.y() + height));
...
impl VoronoiDiagram {
    ...
    pub fn new(min: &Point, max: &Point, points: &[Point]) -> Option<Self> {
        ...
        let clip_points = vec![
            Point::from_xy(min.x(), min.y()),
            Point::from_xy(max.x(), min.y()),
            Point::from_xy(max.x(), max.y()),
            Point::from_xy(min.x(), max.y()),
        ];
    ...
    pub fn from_tuple(min: &(f64, f64), max: &(f64, f64), coords: &[(f64, f64)]) -> Option<Self> {
        ...
        let clip_points = vec![
            Point::from_xy(min.0, min.1),
            Point::from_xy(max.0, min.1),
            Point::from_xy(max.0, max.1),
            Point::from_xy(min.0, max.1),
        ];
...
fn calculate_centroids(
    ...
    for t in 0..num_triangles {
        ...
        centroids.push(Point::from_xy(sum.x / 3., sum.y / 3.));
    }
    centroids
}

fn calculate_circumcenters(
    points: &[Point],
    delaunay: &Triangulation,
) -> Vec<Point> {
    (0..delaunay.len())
